...
const STATUS_VARIANTS: Record<string, string> = {
  ...
  quote_pending: 'bg-purple-50 text-purple-700',
  visit_scheduled: 'bg-orange-50 text-orange-700',
  sale_confirmed: 'bg-success-50 text-success-700',
...
interface ClientCardProps {
  ...
  onWhatsAppClick?: (client: IClient) => void;
...
export const ClientCard = React.memo(function ClientCard({
  ...
  return (
    <div
      onClick={() => onClick?.(String(client._id))}
    ...
    >
      <div>
        ...
        {client.companyName && client.name && client.name !== client.companyName && (
        ...
        <div className="flex items-center gap-1 mt-1">
          ...
          {!displayTemperature && displayScore && displayScore > 0 && (
          ...
          <span className={`inline-flex items-center px-1.5 py-px rounded text-[9px] font-medium ${STATUS_VARIANTS[client.operationStatus || 'none'] || 'bg-gray-100 text-gray-700'}`}>
            {STATUS_LABELS[client.operationStatus || 'none'] || client.operationStatus || 'Sin operación'}
          </span>
        </div>
      </div>
      ...
      <div className="mt-1.5 flex items-center gap-2 text-[11px] md:text-[10px] text-gray-500">
        ...
        <span className="truncate">
          {client.assignedTo
            ? typeof client.assignedTo === 'object' && 'name' in client.assignedTo
              ? (client.assignedTo as { name: string }).name
      ...
      <div className="mt-1.5 flex items-center justify-between text-[11px] md:text-[10px]">
        <span className="text-gray-400">
          {client.createdAt ? relativeTime(client.createdAt as unknown as Date) : '-'}
        </span>
      ...
      {conversationStatus && (
        <div className="mt-1.5 pt-1.5 border-t border-gray-100 space-y-0.5">
          ...
          {/* New message indicator - when client wrote and chat hasn't been opened */}
          {conversationStatus.lastInboundMessageAt && conversationStatus.lastReadAt && (
            (() => {
              ...
              const lastReadTime = new Date(conversationStatus.lastReadAt).getTime();
              const hasUnreadInbound = !isNaN(lastInboundTime) && lastInboundTime > lastReadTime;
      ...
      {conversationStatus?.isHandoffPending && (
        <div className="mt-1.5 flex gap-1">
          <button
            onClick={(e) => { e.stopPropagation(); onTakeCase?.(client); }}
        ...
        <div className="mt-1.5 pt-1.5 border-t border-gray-100 space-y-0.5">
          {client.score !== undefined && client.score > 0 && (
            <div className="flex items-center gap-1.5 text-[10px] text-gray-600">
              ...
              <span>Score: {client.score} pts</span>
            </div>
          )}

...
export function LeadAdminNotesCard({ notes, onSave }: LeadAdminNotesCardProps) {
  ...
  // Keep the persisted value in sync when the parent updates the notes
  // (e.g. after a save from elsewhere) but only when not editing.
  useEffect(() => {
    if (!isEditing) {
      ...
      {isEditing ? (
        <div className="space-y-3">
          <textarea
            ...
            aria-label="Notas privadas del administrador"

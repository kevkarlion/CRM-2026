...
export async function POST(
  ...
  try {
    ...
    const contract = await contractService.changeStatus(params.id, 'cancelled', tenantId, userId);
    if (!contract) {
      return NextResponse.json({ error: 'Contract not found' }, { status: 404 });
    }

    // Cancel future schedules
    const cancelledSchedules = await schedulerService.cancelFutureSchedules(params.id, tenantId, userId);
